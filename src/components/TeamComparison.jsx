...
export function TeamComparison() {
  const [selectedMatch, setSelectedMatch] = useState(null);
  const [prediction, setPrediction] = useState({ team1Score: null, team2Score: null });
  const [loading, setLoading] = useState({ teamStatus: false });
  const [errors, setErrors] = useState({ teamStatus: null });
  const [results, setResults] = useState({ teamStatus: null });
  ...
  const getRadarData = (team, index = null) => {
    ...
    if (results.teamStatus && selectedMatch && index !== null) {
      const teamData = index === 0 ? results.teamStatus.team1Status : results.teamStatus.team2Status;
                  ...
                  <div className="flex flex-col lg:flex-row justify-center items-start gap-8 mb-6">
                    {[selectedMatch.team1, selectedMatch.team2].map((team, index) => (
                      <div key={team.name} className="text-center flex flex-col items-center w-full lg:w-[300px]">
                        ...
                        {errors.teamStatus && (
                          <div className="bg-gray-900 rounded-lg shadow mt-4 p-4 w-full h-[300px] flex items-center justify-center">
                            <p className="text-red-400">Error: {errors.teamStatus}</p>
                          </div>
                        )}
                        
                        {/* Radar chart with data */}
                        {!loading.teamStatus && !errors.teamStatus && (
                          <div className="bg-gray-900 rounded-lg shadow mt-4 p-4 w-full h-[300px]">
                            <Radar
                        ...
                        <div className="mt-4 text-white text-sm">
                          ...
                          {results.teamStatus && !loading.teamStatus && (
                            <div className="mt-2 p-2 bg-gray-700 rounded">
                              <p className="font-bold">Performance Stats (1-10):</p>
                              <p>Attack: {index === 0 ? results.teamStatus.team1Status.attack_norm : results.teamStatus.team2Status.attack_norm}</p>
                              <p>Defense: {index === 0 ? results.teamStatus.team1Status.defend_norm : results.teamStatus.team2Status.defend_norm}</p>
                              <p>Speed: {index === 0 ? results.teamStatus.team1Status.speed_norm : results.teamStatus.team2Status.speed_norm}</p>
                              <p>Teamwork: {index === 0 ? results.teamStatus.team1Status.teamwork_norm : results.teamStatus.team2Status.teamwork_norm}</p>
                              <p>Hustle: {index === 0 ? results.teamStatus.team1Status.hustle_norm : results.teamStatus.team2Status.hustle_norm}</p>
                            </div>
                          )}
                        </div>
                      </div>
